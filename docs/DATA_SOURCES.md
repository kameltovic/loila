...
| `npm run open-data:refs` | Référentiels servis par la prod : IDCC, juridictions compétentes (ministère de la Justice), zones tendues (décret 2013-392), IRL (INSEE, trimestriel). Se lance **en local** ; la prod les reçoit par le bundle `npx tsx scripts/export-content.ts 2026-09-refs --refs` (importé au démarrage, rien à lancer sur le serveur). |
| `npm run legal-jorf` | Journal officiel → graphe. `--links` : liens article ↔ texte JO lus dans le cache LEGI/KALI (créé, modifié, abrogé, déplacé, codifié par, renvois), local. `--meta` : titre officiel, n° et date de parution, NOR, ELI depuis le fonds JORF de la DILA (miroir git quotidien, lecture limitée à l'en-tête). `--decisions` : « loi n° … » / « décret n° … » cités dans les décisions, résolus par numéro et nature (numéros ambigus ignorés). Livré en prod par `export-content.ts <nom> --jorf`. |
