...
// Official lists key Paris, Lyon and Marseille either as one commune or by arrondissement, and so does the
// geocoder: try the code itself, then its counterpart (arrondissement ↔ commune).
export function communeCandidates(citycode: string): string[] {
  if (/^751\d\d$/.test(citycode)) return [citycode, "75056"];
  if (/^132\d\d$/.test(citycode)) return [citycode, "13055"];
  if (/^6938\d$/.test(citycode)) return [citycode, "69123"];
  if (citycode === "13055") return [citycode, "13201"];
  if (citycode === "69123") return [citycode, "69381"];
  return [citycode];
}
